...
struct vector_t {
    ...
    size_t dis() const { return x * x + y * y + z * z; }

    friend size_t dis(const vector_t& lhs,
                      const vector_t& rhs = vector_t(0, 0, 0)) {
        return (lhs - rhs).dis();
    }
    friend size_t manhattan(const vector_t& lhs, const vector_t& rhs) {
...
int main() {
    ...
    for (size_t i = 0; i < scanners.size(); i++) {
        for (size_t j = 0; j < i; j++) {
            maxDis = max(maxDis, manhattan(scanners[i].loc, scanners[j].loc));
        }
    }
    cout << maxDis << endl;
}