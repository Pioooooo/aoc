...
int main() {
    ...
    while (getline(fin, line)) {
        ...
        ss >> word;
        auto d = views::repeat(word | views::transform([](char c) {
                                   switch (c) {
                                       case '.':
                                           return 1;
                                       case '#':
                               ...
                               5) |
                 views::join_with(3);
        auto s = vector<int>(d.begin(), d.end());
        int n, m;
        vector<int> v;
        while (ss >> n >> c) {
            v.push_back(n);
        ...
        v.push_back(n);
        auto vv = views::repeat(views::all(v), 5) | views::join;
        v = vector<int>(vv.begin(), vv.end());
        n = s.size(), m = v.size();
